...
static void att_disconnect_cb(int err, void *user_data)
{
	...
	tGattConnectionEvent event = BT_GATT_DISCONNECT;

	if(gatt_server_cb) {
		gatt_server_cb->GattConnectionEventcb(event);
	}
...
static int l2cap_le_att_accept(int sk)
{
	...
	tGattConnectionEvent event = BT_GATT_CONNECTION;
...
static int gatt_add_service(int argc, void **argv)
{
	...
	tGattAddSvcMsg server_msg;

	server_msg.svc_handle = start_handle;

	if(gatt_server_cb) {
		gatt_server_cb->GattAddSvcCb(&server_msg);
	}
			   ...
			   uint8_t opcode, struct bt_att *att, void *user_data)
{
	 ...
	 *
	 * */
	tGattCharReadReq ChrMsg;
	ChrMsg.attr_handle = attr_handle;
	ChrMsg.is_blog_req = is_long_read;
	ChrMsg.offset = offset;
	ChrMsg.trans_id = trans_id;
	if(gatt_server_cb) {
		gatt_server_cb->GattCharReadReqCb(&ChrMsg);
	}
...
//	memcpy(bt_gatts_req_write.value, value, len);
	...
	if(gatt_server_cb) {
		gatt_server_cb->GattCharWriteReqCb(&ChrWriteMsg);
	}
			   ...
			   uint8_t opcode, struct bt_att *att, void *user_data)
{
	 ...
	 *
	 * */
	tGattDescReadReq DescMsg;
	DescMsg.attr_handle = attr_handle;
	DescMsg.is_blog_req = is_long_read;
	DescMsg.offset = offset;
	DescMsg.trans_id = trans_id;
	if(gatt_server_cb) {
		gatt_server_cb->GattDescReadReqCb(&DescMsg);
	}
...
//	memcpy(bt_gatts_req_write.value, value, len);
	...
	tGattDescWriteReq DescWriteMsg;
	...
	if(gatt_server_cb) {
		gatt_server_cb->GattDescWriteReqCb(&DescWriteMsg);
	}
...
static int gatt_add_char(int argc, void **argv)
{
	 ...
	 *
	 * */
	tGattAddCharMsg AddChrMsg;

	AddChrMsg.char_handle = char_handle;
	AddChrMsg.uuid = argv[1];
	if(gatt_server_cb) {
		gatt_server_cb->GattAddCharCb(&AddChrMsg);
	}
...
static int gatt_add_desc(int argc, void **argv)
{
	 ...
	 *
	 * */
	tGattAddDescMsg AddDescMsg;

	AddDescMsg.desc_handle = desc_handle;

	if(gatt_server_cb) {
		gatt_server_cb->GattAddDescCb(&AddDescMsg);
	}
...
static void conf_cb(void *user_data)
{
	...
	if(gatt_server_cb) {
		gatt_server_cb->GattSendIndicationCb(user_data);
	}
}
