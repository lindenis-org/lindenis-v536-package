...
swupdate_cmd()
{
    while true
    do
        swu_param=$(fw_printenv -n swu_param)
        swu_software=$(fw_printenv -n swu_software)
        swu_mode=$(fw_printenv -n swu_mode)
        ...
        swu_next=$(fw_printenv -n swu_next)
...
[ $# -ne 0 ] && {
    ...
    swu_param=$(echo $swu_input | awk -F ' -e ' '{print $1}')
#    echo "swu_param: ##$swu_param##"
    echo "swu_param $swu_param" > /tmp/swupdate_param_file
    swu_param_last=$(echo $swu_input | awk -F ' -e ' '{print $2}')
    swu_software=$(echo $swu_param_last | awk -F ',' '{print $1}')
#    echo "swu_software: ##$swu_software##"
    echo "swu_software $swu_software" >> /tmp/swupdate_param_file
    swu_mode=$(echo $swu_param_last | awk -F ',' '{print $2}')
#    echo "swu_mode: ##$swu_mode##"
